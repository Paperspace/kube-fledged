...
{{/*
Generate certificates for kubefledged webhook server
*/}}
{{- define "kubefledged.gen-certs" -}}
{{- $altNames := list ( printf "%s.%s" (include "kubefledged.name" .) .Release.Namespace ) ( printf "%s.%s.svc" (include "kubefledged.name" .) .Release.Namespace ) -}}
{{- $ca := genCA "kubefledged-ca" 365 -}}
{{- $cert := genSignedCert ( include "kubefledged.name" . ) nil $altNames 365 $ca -}}
